...
import { retryDelay } from "@haipham/javascript-helper-rxjs";
import { createEventEmitterClient } from "@haipham/javascript-helper-event-emitter";
...
export default function <SubscribeeData>({
  ...
  const eventEmitter =
    createEventEmitterClient<ManyToOnePeerClient.Callbacks<SubscribeeData>>();
  ...
  async function initialize() {
    ...
    if (args.type === "SUBSCRIBEE") {
      ...
      const { connectionOpenTimeout = DELAY_DEFAULT_CONNECTION_OPEN_TIMEOUT } =
        args;
